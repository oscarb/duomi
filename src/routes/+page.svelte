														...
														<span class="text-base font-semibold text-[#2d3142] flex items-baseline">
															{#if currencyConfig.isPrefix}
																{#each formatter.formatToParts(Math.round(item.amount)) as part}
																	{#if part.type === 'currency'}
																		<span class="text-[#9ca3af] opacity-50 font-normal mr-1 inline-block" style="transform: translateY(1px);">{part.value}</span>
																	{/if}
																{/each}
															{/if}
															...
															{#if !currencyConfig.isPrefix}
																{#each formatter.formatToParts(Math.round(item.amount)) as part}
																	{#if part.type === 'currency'}
																		<span class="text-[#9ca3af] opacity-50 font-normal ml-1 inline-block" style="transform: translateY(1px);">{part.value}</span>
																	{/if}
																{/each}
															{/if}
														</span>
