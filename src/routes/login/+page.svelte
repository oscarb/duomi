...
<div class="min-h-[calc(100vh-100px)] flex flex-col justify-center items-center py-12 px-4 sm:px-6 lg:px-8">
	<div class="max-w-md w-full space-y-8 bg-white rounded-2xl p-8 sm:p-10 border border-[#efeeea] shadow-xl transition-all duration-300 animate-slide-in" style="box-shadow: 0 20px 40px -15px rgba(45, 49, 66, 0.15);">
		<div class="text-center">
			...
			<h1 class="text-3xl font-extrabold text-[#2d3142] font-display">
				{t('loginTitle')}
			</h1>
		</div>
		...
		<form class="mt-8 space-y-6 animate-slide-in" method="POST">
			...
			<div>
				<button
					type="submit"
					class="w-full bg-[#ff7361] hover:bg-[#ff7361]/90 active:scale-[0.98] text-white font-bold py-3.5 px-6 rounded-xl shadow-lg shadow-[#ff7361]/25 hover:shadow-xl hover:shadow-[#ff7361]/30 transition-all flex items-center justify-center gap-2 cursor-pointer text-base"
				>
					<span>{t('loginButton')}</span>
					<span class="material-symbols-outlined text-lg" style="font-weight: 300;">arrow_forward</span>
				</button>
			</div>
		</form>
	</div>
