		...
		<div class="lg:col-span-7 flex flex-col space-y-6">
			...
			<div class="flex flex-col space-y-3">
				...
				<div class="bg-white rounded-2xl shadow-[0_8px_30px_rgb(45,49,66,0.04)] border border-[#efeeea] overflow-hidden">
							...
							{#each expensesA as item}
								...
								>
									...
									<div class="text-right">
										<p class="font-bold text-sm text-[#2d3142]">${Math.round(item.currentAmount)}</p>
										<p class="text-[10px] font-bold uppercase tracking-tighter {selectedId === item.id ? 'text-[#ff7361]' : 'text-[#9ca3af]'}">
											{#if item.splitType === 'static'}
												{Math.round((item.staticSplitRatio ?? 0.5) * 100)}% Static
											{:else}
												Dynamic
											{/if}
										</p>
					...
					{:else}
						<p class="text-xs text-[#9ca3af] italic p-6">No active expense templates for {data.personAName}.</p>
					{/if}
			...
			<div class="flex flex-col space-y-3 pt-4">
				...
				<div class="bg-white rounded-2xl shadow-[0_8px_30px_rgb(45,49,66,0.04)] border border-[#efeeea] overflow-hidden">
					...
					{:else}
						<p class="text-xs text-[#9ca3af] italic p-6">No active expense templates for {data.personBName}.</p>
					{/if}
					<div class="px-4 py-3.5 bg-[#fbf9f5]/50 border-t border-[#efeeea]">
						<a
		...
		<div class="lg:col-span-5">
			{#if isCreateMode}
				...
				<div class="bg-white rounded-2xl shadow-[0_8px_30px_rgb(45,49,66,0.04)] p-8 border border-[#efeeea] sticky top-8">
					<h3 class="text-lg font-black text-[#2d3142] font-display mb-6">Create New Template</h3>
					
					<form method="POST" action="?/create" use:enhance class="space-y-6">
						<div class="space-y-1.5">
							<label class="text-[10px] font-black text-[#9ca3af] uppercase tracking-wider" for="new-name">Expense Name</label>
							<input
								id="new-name"
								name="name"
								type="text"
						...
						<div class="grid grid-cols-2 gap-4">
							<div class="space-y-1.5">
								<label class="text-[10px] font-black text-[#9ca3af] uppercase tracking-wider" for="new-paidBy">Paid By</label>
								<select
									id="new-paidBy"
									name="paidBy"
									value={initialPaidBy}
							...
							<div class="space-y-1.5">
								<label class="text-[10px] font-black text-[#9ca3af] uppercase tracking-wider" for="new-interval">Frequency</label>
								<select
									id="new-interval"
									name="intervalMonths"
									class="w-full px-4 py-2.5 rounded-xl border border-[#efeeea] focus:border-[#ff7361] focus:ring-0 text-sm"
						...
						<div class="grid grid-cols-2 gap-4">
							<div class="space-y-1.5">
								<label class="text-[10px] font-black text-[#9ca3af] uppercase tracking-wider" for="new-amount">Amount ($)</label>
								<input
									id="new-amount"
									name="amount"
									type="number"
							...
							<div class="space-y-1.5">
								<label class="text-[10px] font-black text-[#9ca3af] uppercase tracking-wider" for="new-validFrom">Start Month</label>
								<input
									id="new-validFrom"
									name="validFrom"
									type="date"
						...
						<div class="space-y-3">
							<label class="text-[10px] font-black text-[#9ca3af] uppercase tracking-wider block" for="new-splitType">Split Type</label>
						...
						<div class="space-y-1.5">
							<label class="text-[10px] font-black text-[#9ca3af] uppercase tracking-wider" for="new-account">Paid From</label>
							<select
								id="new-account"
								name="accountId"
								class="w-full px-4 py-2.5 rounded-xl border border-[#efeeea] focus:border-[#ff7361] focus:ring-0 text-sm"
				...
				<div class="bg-white rounded-2xl shadow-[0_8px_30px_rgb(45,49,66,0.04)] p-8 md:p-10 border border-[#efeeea] sticky top-8">
					{#if selectedExpense}
						<form method="POST" action="?/update" use:enhance class="space-y-6">
								...
								<div class="flex flex-col items-end">
										...
										>
											<div class="flex items-center">
												<span class="text-2xl font-bold text-[#2d3142] tracking-tight">
													<span class="text-[#9ca3af] mr-1 inline-block" style="width: 1ch; display: inline-block; text-align: right;">$</span>{Math.round(selectedExpense.currentAmount)}
												</span>
											</div>
											<div class="mt-1 text-right flex items-center gap-1">
												<span class="text-[12px] text-[#9ca3af]">since <span class="font-bold text-[#2d3142]">{selectedExpense.history[selectedExpense.history.length - 1]?.validFrom || ''}</span></span>
									...
									{:else}
										...
										<div class="flex flex-col items-end space-y-2">
											<div class="flex items-center">
												<span class="text-2xl font-bold text-[#9ca3af] mr-1" style="width: 1ch; display: inline-block; text-align: right;">$</span>
												<input
													type="number"
													step="1"
											...
											<div class="flex items-center gap-1.5">
												<span class="text-[12px] text-[#9ca3af]">since</span>
												<input
													type="date"
							...
							<!-- Paid By select -->
							<div class="space-y-1.5">
								<label class="text-[10px] font-black text-[#9ca3af] uppercase tracking-wider" for="detail-paidBy">Paid By</label>
								<select
									id="detail-paidBy"
									name="paidBy"
							...
							<div class="pt-2">
								<p class="text-[10px] font-black text-[#9ca3af] uppercase tracking-widest mb-3">Splitting Ratio</p>
								<div class="space-y-4">
									<div class="grid grid-cols-2 p-1 bg-[#fbf9f5] rounded-full border border-[#efeeea]">
										<button
											type="button"
									...
									{#if editSplitType === 'static'}
										<div class="space-y-3 pt-1">
											...
											<div class="flex justify-between text-[10px] font-bold text-[#9ca3af]">
												<span>${Math.round(selectedExpense.currentAmount * editRatio)}</span>
												<span>${Math.round(selectedExpense.currentAmount * (1 - editRatio))}</span>
											</div>
											<p class="text-[10px] text-[#9ca3af] font-medium italic text-center mt-1.5">Adjust the slider to override dynamic income-based splitting.</p>
										</div>
									{:else}
										<div class="flex justify-between text-xs font-bold text-[#2d3142]">
										...
										<div class="flex justify-between text-[10px] font-bold text-[#9ca3af] pt-1">
											<span>${Math.round(selectedExpense.currentAmount * data.dynamicSplitRatioA)}</span>
											<span>${Math.round(selectedExpense.currentAmount * (1 - data.dynamicSplitRatioA))}</span>
										</div>
									{/if}
								</div>
							</div>
							...
							<div class="pt-6 border-t border-[#efeeea]">
								<p class="text-[10px] font-black text-[#9ca3af] uppercase tracking-widest mb-3">Source</p>
							...
							<!-- Frequency Section -->
							<div class="pt-6 border-t border-[#efeeea]">
								<p class="text-[10px] font-black text-[#9ca3af] uppercase tracking-widest mb-3">Frequency</p>
								...
								{#if selectedExpense.nextPaymentDate}
									<div class="flex items-center gap-2 mt-4 px-1">
										...
										<p class="text-[11px] font-bold text-[#9ca3af]">Next payment date <span class="text-[#2d3142]">{selectedExpense.nextPaymentDate}</span></p>
									</div>
								{/if}
							</div>
							...
							<div class="pt-6 border-t border-[#efeeea]">
								<p class="text-[10px] font-black text-[#9ca3af] uppercase tracking-widest mb-3">Price History</p>
										...
										{#each [...selectedExpense.history].sort((a,b) => b.validFrom.localeCompare(a.validFrom)) as hist}
											<tr>
												...
												<td class="py-2 text-right font-bold text-sm text-[#2d3142]">
													<span class="text-[#9ca3af] mr-0.5">$</span>{Math.round(hist.amount)}
												</td>
											</tr>
										{/each}
									</tbody>
							...
							<div class="pt-8 border-t border-[#efeeea] flex items-center justify-between">
								...
								>
									...
									<span class="text-xs font-bold">Archive expense</span>
								</button>
					...
					{:else}
						<div class="py-20 text-center">
							...
							<h3 class="text-lg font-bold text-[#2d3142]">Select a Template</h3>
							<p class="text-xs text-[#9ca3af] max-w-xs mx-auto mt-1">
								Click any expense template on the left to edit its details, ratios, and history, or create a new template.
							</p>
						</div>
					{/if}
				</div>
...
{#if showAddAccount}
	<div class="fixed inset-0 z-50 bg-[#2d3142]/40 backdrop-blur-sm flex items-center justify-center p-4">
		<div class="bg-white rounded-2xl border border-[#efeeea] shadow-2xl p-6 w-full max-w-sm space-y-4">
			<h4 class="text-lg font-black text-[#2d3142] font-display">Add Source Account</h4>
			
			<div class="space-y-4">
				<div class="space-y-1">
					<label class="text-[10px] font-black text-[#9ca3af] uppercase tracking-wider" for="new-acc-name">Account Name</label>
					<input
						id="new-acc-name"
						type="text"
				...
				<div class="space-y-1">
					<label class="text-[10px] font-black text-[#9ca3af] uppercase tracking-wider" for="new-acc-owner">Owner</label>
					<select
						id="new-acc-owner"
